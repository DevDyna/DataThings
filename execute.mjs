...
async function main() {
	...
	if(blackfound > 0){
		bads = `and ${blackfound} ignored files`;
				...
				let file = ''
		if(totfile.length>1){file = 's'}
		console.log(`${totfile.length} File${file} founded ${bads}`)	


	if(io.mkDir('recipes/')){console.log('Directory /recipes/ created')}else{console.log('Directory founded')}
    totfile.forEach(tag => {
			//console.log(`Reading file :${tag}`)
        let ar = io.JsonArray(`tags/${tag}`).values;
        let origin = ar[0];
        ar.shift();
		if(origin == 'chipped:big_lantern' || origin == 'chipped:big_soul_lantern'){return;}
        console.log(origin)
